...
export default function HREmployees() {
  ...
  const canManageEmployeePayroll = Boolean(access.data?.canManagePayroll);
  ...
  const salaryConfigs = trpc.payroll.employees.salaryConfig.useQuery({ employeeId: employeeIdInput }, { enabled: Boolean(selectedEmployeeId) && canManageEmployeePayroll });
  const adjustments = trpc.payroll.employees.salaryAdjustmentHistory.useQuery({ employeeId: employeeIdInput }, { enabled: Boolean(selectedEmployeeId) && canManageEmployeePayroll });
  const payslips = trpc.payroll.employees.payslipHistory.useQuery({ employeeId: employeeIdInput }, { enabled: Boolean(selectedEmployeeId) && canManageEmployeePayroll });
  ...
  return <HRLayout title="員工管理" description="管理員工主檔、薪資設定、調整歷史與已領薪資。薪資調整採有效日期版本化，不會改寫既有薪資條。">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      {canManageEmployeePayroll && <Button className="bg-[#163C72] hover:bg-[#102e59]" onClick={() => setShowCreate((open) => !open)}><Plus className="mr-2 h-4 w-4"/>新增員工</Button>}
    </div>
