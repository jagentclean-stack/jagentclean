...
- [x] 以實際管理員登入後開啟 /cms/menus，驗證頁面可載入且選單資料實際顯示
...
- [x] 前台內容權限控制：僅交付已發布公開內容與白名單設定，並以 SSR 水合與公開 API 回歸測試驗證敏感資料隔離
- [x] 匿名驗證已發布 auth.me 不回傳使用者敏感資料，且 CMS 路由初始 HTML 採 noindex
- [x] 以授權管理員已登入會話驗證 /cms、/cms/prices 與 /cms/users 的讀取及角色保護操作；選單與價格可讀取、使用者管理依權限正確拒絕，未提交任何正式內容變更
...
## CMS 設定與前台同步稽核
- [x] 盤點 CMS Settings 可編輯公開欄位、公開元件與 SSR／React Query 快取資料流，釐清版權聲明未同步原因
- [x] 修正公開 Footer 版權聲明與其他 CMS 公開設定欄位的即時讀取／快取失效機制
- [x] 補齊 CMS 設定儲存後公開前台同步、SSR 輸出與快取更新的回歸測試，並完成正式驗證與發布

## CMS 單一公開設定入口
- [x] 盤點頁尾專屬導覽、介紹與公開設定欄位，界定可整併至網站設定的資料範圍
- [x] 將地址、電話、Email、版權與社群等公開頁尾設定整併至網站設定，移除重複的頁尾設定入口與導覽
- [x] 在網站設定儲存成功後顯示「已儲存並同步至前台」提示，並補齊整併、路由導引與同步回歸測試

## 人事薪資｜借支扣回同步
- [x] 追蹤借支扣回、薪資明細與實發薪資計算流程，釐清本期扣款未帶入的原因
- [x] 修正草稿薪資週期的借支扣回帶入、扣款明細與實發薪資重算邏輯，維持已確認／已發薪週期不可變更
- [x] 補齊借支扣回、重複重算、餘額結轉與薪資週期鎖定的自動化回歸測試，並完成正式驗證與發布

## 人事薪資｜加班、借支與獎金津貼檢視編輯
- [x] 為加班、借支與獎金／津貼提供可檢視明細與可編輯的管理 API，保留前後資料 audit log
- [x] 對連結薪資週期的加班、借支扣回與獎金／津貼編輯套用草稿可修改、已確認／待發薪／已發薪不可直接變更的鎖定規則
- [x] 在加班、借支與獎金／津貼管理頁加入明細檢視、表單預填編輯、儲存回饋與薪資資料更新提示
- [x] 補齊三類資料的權限、編輯、鎖定、稽核、借支扣款與薪資重算回歸測試，並完成正式驗證與發布

## 清潔知識｜首批十篇文章上架
- [x] 盤點文章 CMS 欄位、既有文章與公開發布契約，避免覆蓋既有內容
- [x] 建立並發布使用者提供的十篇清潔知識文章，補齊分類、SEO 摘要、Slug 與發布狀態
- [x] 建立並驗證公開文章列表、個別文章頁、Canonical 與 Article Schema；詳情頁僅公開已發布文章且具備 noindex 隔離規則
